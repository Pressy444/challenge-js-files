...
fetch('https://canvasjs.com/services/data/datapoints.php')
	...
	.then(data => {
		...
		// Vous pouvez les utiliser pour mettre à jour votre graphique
		mettreAJourGraphique(data);
	})
	.catch(error => {
		console.error('Erreur lors de la récupération des données :', error);
	});
function mettreAJourGraphique(data) {
	// Utilisez les données pour mettre à jour votre graphique Chart.js
	// Par exemple :
	const monGraphique = new Chart(document.getElementById('monGraphique'), {
		type: 'bar',
		data: {
			labels: data.labels,
			datasets: [{
		...
		options: {
			// Options de configuration de votre graphique
		}
	});
}
// Exemple : Rafraîchir les données toutes les 5 secondes
setInterval(() => {
	fetch('https://canvasjs.com/services/data/datapoints.php')
		.then(response => response.json())
		.then(data => {
